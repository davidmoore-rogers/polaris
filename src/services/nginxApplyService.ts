 ...
 * Returns the new cert's fingerprint, hostnames, expiry, plus the count of
 * currently-enrolled agents pinned to the CURRENT server cert (those are
 * the ones that will need re-install after rotation).
 */
export async function preflightCertRotation(certPem: string, keyPem: string): Promise<{
  newFingerprint: string;
  newCn: string | null;
  newDnsSans: string[];
  newExpiresAt: string | null;
  agentsToRepinCount: number;
}> {
  validateCertKeyPair(certPem, keyPem);
  ...
  const currentFp = getServerCertFingerprint();
  let agentsToRepinCount = 0;
  if (currentFp) {
    agentsToRepinCount = await prisma.managedAgent.count({
      where: { installStatus: "active", serverCertFingerprint: currentFp },
    });
  }

  return { newFingerprint, newCn: cn, newDnsSans: dnsSans, newExpiresAt: expiresAt, agentsToRepinCount };
}
