 ...
 *
 * Two surfaces gate on the caller's source IP with the same three-way
 * vocabulary: the Dash wallboard (`dashConfig`, an unauthenticated read-only
 * surface) and local login access (`loginAccessConfig`, the /login.html form
 * + the password endpoints). They resolve identically —
 *
 *   "all"     → no gate.
...
import { isPrivateOrLoopbackIp } from "./cidr.js";
import { ipMatchesAllowlist } from "./ipAllowlist.js";

export type IpScope = "rfc1918" | "all" | "custom";
