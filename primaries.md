...
- **Integration edit modal** — class-subtab strip (FortiGates / FortiSwitches / FortiAPs for FMG+FGT; single class for AD/Entra/WinSrv) → stream-subtab strip per class. Uses `_classStreamSubtabHTML(..., isPrimary=true)` for the active class (legacy DOM ids `f-mon-tier-<pollField>`) and `isPrimary=false` with namespaced `f-mon-classecho-<klass>-` prefix for echo subtabs. `showInherit: true`, `showMib: true` (defaults).
...
- **LLDP + Storage cadence at the asset tier:** the Asset row has no `lldpIntervalSec` / `storageIntervalSec` columns. Both streams share `systemInfoIntervalSec` / `systemInfoTimeoutMs` with Interfaces. Subtabs for LLDP + Storage on the asset modal render an inline hint pointing operators at the Interfaces subtab. The integration / manual tiers persist `lldpIntervalSeconds` + `storageIntervalSeconds` separately (inert today, wired in a future release).
