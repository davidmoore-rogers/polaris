...
- `CLAUDE.md` → Authentication & RBAC → "FMG auth note" (never call `/sys/logout`; commit `3e70476`; the `rpcInner` transient-vs-permanent retry policy).
- `CLAUDE.md` → key coding conventions → "FortiManager ↔ standalone FortiGate parity".
- `TOUCHES.md` → `services/fortimanagerService.ts` and `services/fmgWorker.ts` per-service entries.
