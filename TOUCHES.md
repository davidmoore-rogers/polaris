...
- `agent/cmd/polaris-agent/main.go` — host-side Go binary; loads agent.conf, runs /enroll on first boot (persists returned bearer back to agent.conf via Save), then ticks the response-time collect loop + heartbeat loop + outbound WS loop until SIGTERM. Generic across deployments; per-install identity (server URL, cert pin, bearer) lives entirely in agent.conf. Agent runtime lives in `runAgent(ctx, confPath)` so the Windows Service handler can call it under the SCM.
...
**Public API:** `compileWildcard`, `compilePattern`, `resolvePinnedInterfaces`, `mergeTunnelsIntoInterfaces`, `getInterfaceAggregate`, `previewAutoMonitorForClass`, `applyAutoMonitorForClass`, `coerceLegacySelection`, `AutoMonitorSelection`, `AutoMonitorClass`, `ResolverInterface`, `TunnelObservation`, `LldpNeighborMatch`, `LldpByIfName`, `LldpNeighborType`, `IfType`, `AggregateRow`, `PreviewResult`, `ApplyResult`, `LLDP_NEIGHBOR_TYPES`, `IF_TYPES`.
...
- **IPsec tunnel augmentation (fortigate only):** `loadLatestInterfaces(ids, includeIpsecTunnels)` is called with `includeIpsecTunnels = klass === "fortigate"` by all three callers (`getInterfaceAggregate` / `previewAutoMonitorForClass` / `applyAutoMonitorForClass`). When set, `mergeTunnelsIntoInterfaces` appends one synthetic `{ ifName: tunnelName, ifType: "tunnel", operStatus }` per tunnel, **de-duped per asset against real interface ifNames** so a tunnel SNMP already captured via IF-MIB (ifType 131) isn't double-counted. Status→operStatus maps only fully-`down` → `"down"` (up/partial/dynamic/null → `"up"`) so the `onlyUp` filter keeps healthy-but-not-fully-up tunnels. Effect: the synthetic rows flow into both the aggregate pickers AND the resolver feed, so a tunnel selected By name/By type actually pins into `Asset.monitoredInterfaces`. Pinning yields IF-MIB counters on SNMP and nothing on REST; tunnel SA-status/throughput stay in the dedicated IPsec stream (`monitoredIpsecTunnels` → `asset_ipsec_tunnel_samples`). The helper is pure — mutate-in-place + return — and unit-tested in `tests/unit/autoMonitorInterfacesService.test.ts`.
...
- **Update source repo is configurable.** `ensureUpdateRemote()` runs before the fetch in `checkForUpdates()` AND before the pull in `applyUpdate()`. It repoints the `origin` remote at `POLARIS_UPDATE_REPO` (env), falling back to the hardcoded canonical upstream `https://github.com/davidmoore-rogers/polaris.git` when unset. Idempotent (only rewrites when the URL differs; `git remote add`s if origin is missing) and non-fatal. The configured-or-default URL ALWAYS wins over a fork's existing origin. The two fallback scripts (`deploy/update-linux.sh`, `deploy/update-windows.ps1`) read the same `.env` var and repoint origin in lockstep — keep all three in sync when changing the default repo or the env-var name.
