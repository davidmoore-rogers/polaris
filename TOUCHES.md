...
**What it owns:** The single windowed failed-probe-ratio SQL over `asset_monitor_samples` (UTC-wall-clock-anchored `now() AT TIME ZONE 'UTC'` window, measured from each asset's first successful probe inside it, grouped per asset).
...
**When changing this:** both consumers' post-processing assumes `{ assetId, total, failed }` bigint rows; the window must stay UTC-wall-clock-anchored (the hypertable's timestamps are naive UTC). Behavior is covered by `tests/integration/probeLossQuery.test.ts` (real SQL against Postgres) plus the SQL-shape assertions in `tests/unit/nocDashboardService.test.ts`.
