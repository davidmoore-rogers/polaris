...
- Compared fields: hostname, serialNumber, manufacturer, model, os, osVersion, learnedLocation, ipAddress, latitude, longitude (match `ProjectedAsset` keys). latitude/longitude are SKIPPED when `Asset.coordSource === "manual"`, and hostname is SKIPPED when `Asset.hostnameOverride` is set — an operator pin deliberately diverges from the projection (the db.ts guard re-asserts it over discovery writes), so it's not drift.
