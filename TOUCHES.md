...
- `fortigate-firewall` carries `mgmtMac` — the management-interface MAC read from `/api/v2/cmdb/system/interface`'s `macaddr` field on the SAME query that resolves `mgmtIp` (zero extra REST calls), in `fortigateService.ts` (standalone + FMG-direct, which delegates here) AND the FMG proxy-lane interface query in `fortimanagerService.ts` (`fields:["name","ip","macaddr"]`). Normalized via `normalizeMacOrNull` (`src/utils/mac.ts` — colon-uppercase, all-zero → null). Stamped on `Asset.macAddress` + `macAddressRows` at create AND reconciled into `AssetMacAddress` on update (mirrors the `fortiswitch` baseMac pattern), **primary HA member only** (`memberDevice.mgmtMac` is undefined for the standby — the resolved MAC belongs to the active box). Purpose: get the firewall into discovery's in-memory `byMac` index so Phase 7 device-inventory recognizes it and never spawns a duplicate `fortigate-endpoint` ghost for the firewall's own mgmt MAC. `mgmtMac` is also recorded in the observed blob for the Sources tab but is NOT projection-owned (`macAddress` is written directly, like `learnedLocation`). Pre-existing firewall ghosts converge via `mergeDuplicateHostnameAssets`.
