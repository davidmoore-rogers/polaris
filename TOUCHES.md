...
- `src/api/routes/agents.ts:POST /samples` — the Polaris Agent ingestion path: `enqueueMonitorSample` (responseTime, also via `recordProbeResult`), `enqueueTelemetrySample` + `enqueueTemperatureSamples` (telemetry), `enqueueInterfaceSamples`, `enqueueStorageSamples`. **This writer runs on the `web` role**, not monitor — which is why the web role must also run the flush tick (see Boot).
