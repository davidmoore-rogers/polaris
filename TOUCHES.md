...
- `src/api/routes/assets.ts` — Three endpoints: `GET /assets/:id/dependencies`, `PUT /:id/dependencies/override` (admin, with cycle validation), `DELETE /:id/dependencies/override` (admin). The GET's effective/computed parent split counts every non-`override` source, and its children read is capped per half (`DEP_INFRA_CHILD_CAP` 300 / `DEP_ENDPOINT_CHILD_CAP` 100, `childrenTruncated` + per-row `childCount` in the payload) with grandchildren restricted to infra types — a site gate is the last-seen device for every endpoint at the site, so an uncapped read would put the whole site in one modal payload.
- `public/js/assets.js:renderDependencyTreeBlock()` — General-tab tree, now rendered for EVERY asset type (it used to return "" for non-infra). Endpoint rows carry a `detectedVia` tag ("last-seen switch port" / "last-seen access point" / "last-seen firewall") because an edge that silences alerts should say which signal put it there; an endpoint with no resolvable upstream gets an explicit "dependency suppression can't apply" empty state rather than a missing block.
...
- Fully-down assets (0 successes in the window) are excluded in BOTH modes — asset-down alerting/widgets own them. Post-anchor this falls out of `"firstOk" IS NOT NULL` rather than a HAVING, so engine mode carries no HAVING at all.
...
- The endpoint half makes a site FortiGate's child set fleet-sized. `GET /assets/:id/dependencies` caps children (`DEP_INFRA_CHILD_CAP` / `DEP_ENDPOINT_CHILD_CAP`) and keeps grandchildren infra-only; any new consumer that walks children needs its own bound.
...
- **Fast-confirm re-probe (business rule 30):** `resolveProbeIntervalSec` is the ONE probe-spacing decision, shared by `computeDueWork` (cursor pass) and `publishDueWork` (pg-boss) — their due-sets are contractually identical, so never re-inline the arithmetic. Bounded by construction (≤ `failureThreshold - 1` fast probes per episode, since a probe zeroes the opposite counter); clamps in order = dependency-suppressed 2× → never above the configured cadence → floored at `max(probeTimeoutMs, PROBE_LOOP_TICK_SEC 5s)` so a re-probe can't overlap an in-flight one and double-count a miss. Excludes agent/disabled streams. `fastConfirmIntervalSec` is a tier-2+ hierarchy field (no per-asset column, mirroring `failureThreshold`); the UI's "Declare Down after (seconds)" is DERIVED via `window._polarisMonDownAfterCalc` and writes `failureThreshold` — it is never stored.
...
- **The MAC forwarding table is decoded, not read.** Both FDB tables carry the MAC in the OID INDEX (there is no macAddress column) and report a `dot1dBasePort`, never an ifIndex. `src/utils/macForwarding.ts` owns both: `parseFdbIndex` tells Q-BRIDGE from BRIDGE by index COMPONENT COUNT (7 = fdbId+MAC, 6 = MAC), and `basePortToIfName` composes `dot1dBasePortIfIndex` with the IF-MIB walk. **Omitting that composition is the silent failure** — every MAC lands on whatever interface sits at that ifIndex, which looks like data. `status` is load-bearing: `invalid` is dropped at collection (an aged-out entry would put MACs on ports they have left), and `self` is the REPORTING BRIDGE'S OWN address, excluded from every count and inference — a peer switch appears as a `learned` row on the port facing it, never as `self`.
