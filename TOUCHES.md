...
- **`METRIC_DIMENSIONS` and `FIELD_DIMENSIONS` are the per-metric / per-state-field dimension vocabularies** (published as `metricDimensions` / `fieldDimensions` on `/schema`; validated against by `notificationDimensionService`). Every `ASSET_METRICS` entry gets **`hostnamePattern`** appended (the one dimension naming the DEVICE rather than a sub-asset — substring vs `Asset.hostname`, applied by the engine's `applyHostnameDimension` asset-set filter before any sample query; it exists so a composite tree can mix host-specific branches in one automation, and it rides `triggerSignature` like every dimension so same-metric rules with different hostname filters never carve each other out). `FIELD_DIMENSIONS` is what makes the wizard render dimension inputs on asset_state leaves at all — the engine honored `ifNamePattern` on the ifOper/ifAdmin/poe trio and `tunnelName` on ipsecStatus before the builder could author them (2026-08); the SD-WAN state pair carries only `hostnamePattern` because the engine has no rule-name filter for it.
...
**Depends on** — `notificationEngine.loadScopeAssetIds` (the SAME `loadScopeAssets` the evaluation tick uses — deliberate, so the picker can never offer a value from a device the automation wouldn't evaluate) and `notificationTypes.METRIC_DIMENSIONS` **or `FIELD_DIMENSIONS`** — the `metric` param may also name an asset_state FIELD (`ifOperStatus` …), the two namespaces being disjoint by construction; a mismatch is a 400, never a silent empty list that reads as "no sensors".

**Invariants**

- **`DIMENSION_SOURCES` keys and the `dimensionFilterSchema` fields are a LOCKSTEP PAIR.** Adding a dimensionFilter field means: the Zod field, `METRIC_DIMENSIONS` (and/or `FIELD_DIMENSIONS` for state fields), `dimensionPhrases` (server) + `DIM_PHRASE`/`DIM_PLACEHOLDER`/`FORMULA_DIM` (client), the engine's reading resolver (the filter is only real if something applies it), and a `DIMENSION_SOURCES` entry — otherwise the input silently stays free text. `hostnamePattern`'s source lists the SCOPED devices' own hostnames (Asset rows, not a sample table — `since` unused); its resolver-side application is `applyHostnameDimension` in the engine, shared by both asset resolvers.
