...
- **HTTP Check (`http`) requires NOTHING** — an unauthenticated `GET /` expecting any 2xx is a valid check, so validation is about the check being MEANINGFUL rather than complete. `path` is canonicalized to a leading slash and a blank stores as `"/"` (deliberately unlike `Asset.httpCheckPath`, where blank means "no override"); `port`/`expectStatus` are coerced to numbers and range-checked; a `regex` match pattern is COMPILED at save, because an invalid one would otherwise fail once per asset per interval forever with the operator finding out from a probe error column instead of the form they typed it into (a bare bracket in `contains` mode is legitimate text and is NOT compiled); basic auth needs BOTH halves, since a username alone sends an empty credential that reads as an unexplainable auth failure. **`authMode`** (`none` | `bearer` | `basic` | `digest`) is validated against the mode the probe will actually RESOLVE (`resolveHttpAuthMode`, utils/httpCheck.ts) rather than against whichever fields happen to be filled in, so the form cannot save a credential that authenticates differently from how it reads; an ABSENT value is not "none" but a pre-field row, and infers the old precedence (apiToken → bearer, else a full username/password pair → basic) so a stored credential keeps authenticating exactly as it did. Digest exists as an explicit mode rather than an inference because Basic and Digest ride the SAME username/password pair — guessing (try Basic, fall back on 401) would send the password in cleartext to any device that wanted Digest. `validateHttpConfig` also STRIPS the carriers the resolved mode doesn't send, and that placement is load-bearing: it runs on the MERGED config (validateConfig is called after `mergeConfigPreservingSecrets` on both the create and update paths), which is the only place a stored secret can actually be removed — blanking a secret in the request body means "keep the stored value" to the merge, so a client-side clear would preserve the very credential it appears to delete, leaving a stale bearer token sealed in the row and live again the moment anyone switched the mode back. Secret fields are `apiToken` + `password` — `expectBody` is deliberately NOT masked, being the thing the operator must see and edit on every visit. Semantics + the reasoning behind each default: `utils/httpCheck.ts`, business rule 33.
...
- **`http` (HTTP Check) is the one probe whose SUCCESS CRITERION is operator-defined** (business rule 33): `probeHttp` owns only the socket and hands the status code + body to the pure `evaluateHttpCheck` (`utils/httpCheck.ts`), so a 200 carrying the wrong body is a FAILURE naming the missing content (`failOnMismatch`, default true — an absent value is the strict reading). It takes the same credential chain as snmp/ssh/winrm (per-stream asset cred → asset default → class-override tier) but has **no integration fallback and no bare default** — a missing `http` credential is a probe failure naming the fix, since a silent `GET /` would report a login page as healthy. It reads `Asset.httpCheckPath` (path-only override; blank = no override, NOT "/") and is the only non-ICMP method added to `probeAsset`'s name-fallback list, because a TLS check against a NAME is the check an operator means. Redirects are never followed and the body is capped at 64 KB with the request torn down at the cap. `netGuard` is deliberately NOT applied here — see business rule 33(f). It is also the only probe that may issue TWO requests: `authMode: "digest"` cannot pre-authenticate (the response hash is keyed on a server-issued nonce), so `probeHttp` fires bare, parses the 401 challenge (`utils/digestAuth.ts`) and re-fires ONCE with the computed response — never a loop, and the second request inherits what REMAINS of the probe timeout rather than a fresh one, so the whole exchange stays inside the configured window. When the challenge can't be answered (the device offered Basic only, or a qop/algorithm the client refuses to guess at) the reason is appended to the probe error instead of leaving a bare 401, and `HttpProbeDiagnostics` carries `authRequested` (the scheme NAMES only — no realm, no nonce) + `digestNegotiated` so the Test Connection modal can say "this device asked for Digest and the credential is set to Basic", which is otherwise indistinguishable from a wrong password.
