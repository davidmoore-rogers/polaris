...
- An endpoint gets ONE parent (most-specific of switch-port → wireless → firewall sighting), never a union. All-down semantics are for redundancy; a switch and its gate are in series, so listing both would let a dead switch under a healthy gate keep the endpoint alerting. Series propagation comes from the switch's own suppression instead.
...
- Adding a signal to the endpoint half? It's one parent per endpoint by design. A new signal takes a place in the precedence ladder — it does not become a co-parent — and it must be device-scoped enough that a stale value can't point at a gate the device was never behind.
...
**Public API:** `listContacts`/`getContact`/`createContact`/`updateContact`/`deleteContact`, `normalizeContactEmail`, `normalizeContactCondition`, `conditionMeansAllDevices`, `contactFilterOf`, `previewContactAssets`, `searchAddressBook`, `resolveContactsForAsset`/`resolveContactEmailsForAsset`, `bumpContactCache`.
...
**Public API:** `queryProbeLossRatios({ sinceMinutes, assetIds?, onlyLossy?, limit?, includeFullyDown? })`, `ProbeLossRow`.
...
**Invariants:**
- **The measurement starts at the first successful probe in the window** (`min(s."timestamp") FILTER (WHERE s."success") OVER (PARTITION BY s."assetId")`; older samples trimmed by the outer `WHERE`). Loss is what happened *while the device was answering* — the unreachable part of a window is an outage, which asset-down owns. Without it a device recovering from a long outage alerts *because* it came back, and a 60-minute window makes that stick for an hour (business rule 29).
- **...and no earlier than the last outage's end** — `GREATEST("firstOk", "recoveredAt")`, where `recoveredAt` is `Asset.recoveryStartedAt` carried in by a **LEFT JOIN** to `assets`. This is the case the first anchor cannot see: once the device has been back long enough for a healthy sample to precede the outage, the window's first success sits BEFORE it and the anchor is inert, so the whole outage is back in the denominator (business rule 29b). Three things to keep: the join must stay LEFT (the sample tables have no FK to `assets`, and a row whose asset is gone still counts — several tests seed samples with no asset row at all); `GREATEST` is chosen for its NULL-ignoring semantics, so **never** rewrite it as a `CASE`/`COALESCE` pair that treats NULL as "now" or as the epoch; and the writer's side of the contract — only an outage-ending recovery stamps that column — lives in `recordProbeResult` via the pure `utils/probeLossAnchor.ts` (`stampsRecoveryAnchor`). The reader has a JS twin, `effectiveLossAnchorMs`, used by `alertChartService`'s loss chart; if the SQL anchor changes, change it too or the emailed chart contradicts the reading that fired.
- **`includeFullyDown` is DISPLAY ONLY.** It reports an asset with no successful probe in the window as 100% (total = failed) instead of letting the missing anchor drop it -- before it, a device down for a whole window VANISHED from the Packet Loss widget, which reads as "no loss". The engine must never pass it: `assetIsAnsweringProbes` already keeps `down`/`recovering` assets out of its id list, and a 100% reading reaching a `probeLossPct` automation fires exactly the second alert business rule 29 exists to prevent. If you ever pass it on the engine path, that dedup is gone.
- **This query counts EVERY `probeKind`** -- one of only two all-kinds readers of `asset_monitor_samples` (with `alertChartService`'s loss chart); every response-time reader filters to primary. That is the entire point of the ICMP loss sampler (`utils/lossSampler.ts`), whose rows carry `probeKind="icmp"` + a NULL `responseTimeMs` and so can only ever move a ratio, never a timing. A sampler ping is also a legitimate ANCHOR -- the anchor asks "did anything answer", not "which probe did".
- Fully-down assets (0 successes in the window) are excluded in BOTH modes unless `includeFullyDown` — asset-down alerting/widgets own them. Post-anchor this falls out of `"firstOk" IS NOT NULL` rather than a HAVING, so engine mode carries no HAVING at all.
...
**When changing this:** both consumers' post-processing assumes `{ assetId, total, failed }` bigint rows; the window must stay UTC-wall-clock-anchored (the hypertable's timestamps are naive UTC). Behavior is covered by `tests/integration/probeLossQuery.test.ts` (real SQL against Postgres — including the recovery anchor, and the without-the-stamp case that pins WHY it exists) plus the SQL-shape assertions in `tests/unit/nocDashboardService.test.ts` (which assert the anchor expressions verbatim, so an aliasing change lands there too) and the pure halves in `tests/unit/probeLossAnchor.test.ts` / `tests/unit/probeLossWindow.test.ts`.
...
**Public API:** `resolveRecipientUsers(recipientTags)`, `resolveRecipientUsersByIds(ids)`, `resolveEmailRecipients({recipientUserIds?, addresses?})`, `dedupeEmailRecipients(to, cc, bcc)` (pure), `listRecipientUsers()` (builder picker), `scopeRegionTagsOf(scope)`, `expandDeliveries(notificationId, targets, opts {scopeRegionTags?, assetRegionTags?, composedEmail?, escalation?, excludeUserIds?, suppressedOut?})`, `resolveTargetUsers(target, ctx)` (the per-target recipient union, extracted so the escalation suppression can't drift from the delivery it suppresses), `resolveReachableUserIds(targets, ctx)` (who those targets would ACTUALLY deliver to), `ExpandDeliveriesOptions` + `RegionRoutingContext` + `ComposedEmail` types, `bumpRecipientIndex()`.
...
- `excludeUserIds` (escalation's highest-tier-wins) filters the resolved USER union only — typed `addresses` and address-book contacts pass through, because they carry no user id to defer and an operator typing an address is an explicit choice to page that mailbox. `suppressedOut` is an out-param (the httpCheck-diagnostics pattern) counting DISTINCT users dropped, populated only when passed; the escalation sweep needs it to tell a dead channel apart from a tier whose audience a higher tier owns.
- `resolveReachableUserIds` answers REACHABLE, not merely named: enabled channels only, email needs a non-empty `User.email`, web_push needs ≥1 `PushSubscription` (one grouped read for the whole set), fixed-destination transports contribute nobody. It feeds a suppression that SILENCES an earlier tier, so a false "they'll hear about it later" loses the recipient entirely — keep it stricter than `resolveTargetUsers`, never looser.
...
**Public API:** `buildAlertCharts(assetId, tokens, {now?, thresholds?, lossWindowMs?})`, `chartTokensIn(...templates)`, `substituteChartTokens(body, charts, {html})`, `attachmentsFor(charts, body)`, `lossBucketMs(windowMs)`, `probeLossSeriesFrom(rows, bucketMs)`, `CHART_TOKENS`, `CHART_WINDOW_MS`.
...
**Public API:** `runEscalationSweep(now?)` (job entry), `tierIsDue(tier, triggeredAt, tierState, now)` (pure, structural tier type — legacy and v2 tiers both fit; unit-tested). The companion `higherEscalationTiers(tiers, idx)` is pure and lives in `notificationTypes` beside the other chain helpers.

**Cross-service deps:** `prisma` (rules + notifications + assets), `automationActionService.executeActions` (the entire send path — this service no longer builds delivery rows itself), `notificationTypes.normalizeRuleToV2` + `higherEscalationTiers` + `actionsToTargets`, `notificationRecipientService.scopeRegionTagsOf` + `resolveReachableUserIds`, `notificationEngine.isSuppressedForNotifications`, `src/utils/notificationTemplate.ts` (buildTemplateContext/formatElapsed/notificationsPageUrl), `eventLogService.logEvent`.
...
- A tier counts as SENT (state bump) when `executeActions` reports ≥1 executed action — dead channels / empty recipients retry next sweep — OR when it reports `suppressed > 0` and delivered nothing, i.e. a higher tier owns every person on it. Without that second arm the ordinary "escalate to a wider group" ladder (tier 2's audience a superset of tier 1's) leaves tier 1 resolving to nobody and being retried every 60s for the life of the alert.
- **Highest tier wins.** Before a due tier runs, `laterTierReach` asks `resolveReachableUserIds` who the HIGHER tiers of the SAME chain will reach and passes them as `exec.excludeUserIds`. "Higher" is `higherEscalationTiers` — greater `afterMin`, array index only as a tie-break, because tier order is not validated as ascending and a 5-minute tier authored last would otherwise read as the chain's highest and silence everything above it. Three boundaries are deliberate: chains never cross (the level chain and each per-action chain are independent ladders), the engine's own FIRE is never suppressed (escalation is what repeats, not the page that opened the incident), and only reachable ACCOUNTS count. Known trade-off: an alert acknowledged or cleared before the higher tier comes due never pages the suppressed recipient — that is the shape "only the higher escalation" asks for.
- The reach lookup is memoized per (rule, chain, tier, sorted region snapshot) for the pass, caching the PROMISE so concurrent lookups coalesce: a 200-alert outage on one rule resolves it a handful of times, not 200.
...
**When changing this:**
- Adding a source kind: add it to `LOCATION_CONTRIBUTORS` in `utils/assetSourceLocation.ts` (declaration order IS `DEFAULT_LOCATION_ORDER`; put a new kind at the END unless you mean to change the default for every install, since the normalizer appends unmentioned kinds to stored orders in that same order) and to the `learnedLocation` row of ARCHITECTURE.md's projection priority table. No client change needed; the settings list is server-driven.
...
**What it owns:** Named-credential store for monitoring probes (SNMP v2c/v3, WinRM, SSH, REST API, HTTP Check); type-specific config validation; secret masking on GET; merge-and-preserve logic for PUT to retain secrets when client resubmits mask. Also the credential-usage resolver (where is a credential wired across the monitor-settings tiers).
...
- **HTTP Check (`http`) requires NOTHING** — an unauthenticated `GET /` expecting any 2xx is a valid check, so validation is about the check being MEANINGFUL rather than complete. `path` is canonicalized to a leading slash and a blank stores as `"/"` (deliberately unlike `Asset.httpCheckPath`, where blank means "no override"); `port`/`expectStatus` are coerced to numbers and range-checked; a `regex` match pattern is COMPILED at save, because an invalid one would otherwise fail once per asset per interval forever with the operator finding out from a probe error column instead of the form they typed it into (a bare bracket in `contains` mode is legitimate text and is NOT compiled); basic auth needs BOTH halves, since a username alone sends an empty credential that reads as an unexplainable auth failure. Secret fields are `apiToken` + `password` — `expectBody` is deliberately NOT masked, being the thing the operator must see and edit on every visit. Semantics + the reasoning behind each default: `utils/httpCheck.ts`, business rule 33.
...
- **The ICMP loss sampler must never touch the state machine — and is CURRENTLY DISABLED.** The `LOSS_SAMPLER_ENABLED` kill switch in `utils/lossSampler.ts` (false since 2026-08-20 — the failure-period oversampling bias made alert readings diverge from wall-clock chart averages) defaults `lossSamplerAppliesTo` off, so both due-set builders AND `runLossSampleFor`'s pickup re-check collect nothing; everything below stays wired for a one-line re-enable, and the unit tests exercise the window with an explicit `enabled=true`. `runLossSampleFor` + `utils/lossSampler.ts` own the `lossSample` cadence: one 5s-timeout ping every 10s while `monitorStatus` is `warning` or `recovering`, writing an `AssetMonitorSample` with `probeKind="icmp"` and a NULL `responseTimeMs` via `enqueueMonitorSample` and stamping `Asset.lastLossSampleAt` on EVERY attempt (so a ping-blocked host is not re-queued each tick). It deliberately does NOT call `recordProbeResult`: ICMP does not authenticate the device it reaches, so letting it vote on `down` would false-`down` every ICMP-blocked host on its first transient miss and would leave a dead-SNMP-agent host flapping `warning`<->`up` forever. `down` is outside the window because there every response-time poll is failing, leaving a ping reply uncorroborated -- a squatter on the address would drag a dead device's loss from 100% down to something reading like congestion. The runner RE-CHECKS the window at pickup (under pg-boss the job can land seconds later) and both due-set paths mirror the predicate; keep them in lockstep like every other cadence. In CURSOR mode the due-set is capped at `LOSS_SAMPLES_MAX_PER_PASS` (200, logged when it truncates): `runMonitorPass` awaits the whole pass under the light tick's `running` guard, so an uncapped burst during a site-wide outage would delay the very probes that decide whether those assets are down. pg-boss mode needs no cap — own pool, 15s job expiry.
...
- **`http` (HTTP Check) is the one probe whose SUCCESS CRITERION is operator-defined** (business rule 33): `probeHttp` owns only the socket and hands the status code + body to the pure `evaluateHttpCheck` (`utils/httpCheck.ts`), so a 200 carrying the wrong body is a FAILURE naming the missing content (`failOnMismatch`, default true — an absent value is the strict reading). It takes the same credential chain as snmp/ssh/winrm (per-stream asset cred → asset default → class-override tier) but has **no integration fallback and no bare default** — a missing `http` credential is a probe failure naming the fix, since a silent `GET /` would report a login page as healthy. It reads `Asset.httpCheckPath` (path-only override; blank = no override, NOT "/") and is the only non-ICMP method added to `probeAsset`'s name-fallback list, because a TLS check against a NAME is the check an operator means. Redirects are never followed and the body is capped at 64 KB with the request torn down at the cap. `netGuard` is deliberately NOT applied here — see business rule 33(f).
...
**Invariants:**
- **`probeKind` decides who may read a row.** NULL (and `"primary"`) = the response-time poll; `"icmp"` = the loss sampler. Loss counts every kind; every response-time reader filters to primary, INCLUDING all rollups — so a sampler row never outlives the detail retention window. If you add a reader of `asset_monitor_samples`, decide which side it is on before you write the query (the primary-only list is in ARCHITECTURE.md's AssetMonitorSample block).
