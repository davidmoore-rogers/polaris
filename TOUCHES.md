...
- **Index-named sensors get a display-only interface, never a new key.** `collectHardwareSensorsSnmp`'s ENTITY-SENSOR path names a row `sensor-<entPhysicalIndex>` when the device leaves `entPhysicalDescr` empty (every FortiSwitch row). `annotateSyntheticSensorIfNames` then stamps `HardwareSensorSample.ifName` from a per-target 15-min-cached `ifName`/`ifDescr` walk (`ifNameByIndexCached`), gated on `manufacturer =~ /fortinet/i` because entPhysicalIndex↔ifIndex equivalence is a FortiSwitchOS property, not RFC 4133 — on other vendors the same math would label a PSU with a random port. Consumers: `recordHardwareSensorResult` → `AssetHardwareSensorSample.ifName` → `GET /assets/:id/system-info` → the Hardware Sensors table + sensor slide-over title + mobile asset sheet (parenthesized suffix). **Never fold it into `sensorName`**: that string is the automation `sensorNamePattern` dimension, the rollup tables' unique key, and the chart series key, so a one-tick IF-MIB failure would fork a sensor's history. Rollup tiers deliberately don't carry it. Pure resolution + the synthetic-name test live in `src/utils/hardwareSensors.ts` (`syntheticSensorIndex` / `resolveSensorIfName`).
