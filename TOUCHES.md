...
**Used by:**
- `src/services/discovery/discoveryEngine.ts` Phase 2a — calls `applyFirewallDecommission(hostname)` per stale firewall after the status flip.
- `src/services/discovery/discoveryEngine.ts` Phase 3 firewall create — calls `seedFirewallTagRegistry(fgHostname)` after the `prisma.asset.create` so the picker carries the tag from day one.
- `src/services/discovery/discoveryEngine.ts` Phase 3 firewall update — calls `applyFirewallRename(old, new)` when projection writes a different hostname.
- `src/services/discovery/discoveryEngine.ts` Phase 13.5 — calls `reconcileFirewallTagsForIntegration(integrationId)` after the Phase 13 map-region pass (`mode in {"full", "finalize"}`).
