...
- **A windowed-ratio metric's `windowSec` is used EXACTLY** (`probeLossPct`; `WINDOWED_RATIO_METRICS` in notificationTypes, published as `windowedRatioMetrics`): `PROBE_LOSS_MIN_WINDOW_SEC` (5 min) floor, `PROBE_LOSS_DEFAULT_WINDOW_SEC` (15 min) when the trigger carries none — NOT `max(windowSec, DEFAULT_LOOKBACK_MS)`, which exists so a `latest` reading finds a recent sample and would silently measure a 5-minute History over 15. The wizard's one time field is the window for these (label "History"), never a hold clock.
...
- The PACKET-LOSS chart is derived, not stored: `AssetMonitorSample` records one success/failure per probe, so `loadProbeLoss` buckets them into 2-minute windows and plots the failure ratio. Empty buckets are skipped rather than plotted as 0 — a gap in polling is not a period of perfect health.
