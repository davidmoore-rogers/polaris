...
- All 25 tables can be Timescale hypertables (`timescaleService.ALL_HYPERTABLE_CANDIDATES` = the 24 tiered + the standalone `asset_custom_widget_samples`); plain Postgres works just as well, just without chunk-drop pruning. (`tests/unit/timescaleTables.test.ts` drift-guards the inventory: rollup-writer / prune-layer / schema table references must stay ⊆ the managed lists.)
...
- `src/services/capacityService.ts:projectSteadyStateSize` — enumerates all 24 tables and multiplies retention × rows/asset/day × **calibrated `DEFAULT_BYTES_PER_ROW`** per (stream, tier, default class) for the steady-state footprint projection. It deliberately does NOT use the live-measured `avgBytesPerRow` (relpages ÷ pg_stat tuples) — that's unreliable for compressed/bloated hypertables and produced phantom 14–218 TB projections (2026-06). Exported + unit-tested (`tests/unit/capacitySteadyState.test.ts`).
...
**What it owns:** TimescaleDB extension detection and hypertable migration for the eight sample tables + sixteen rollup tables + the standalone detail-only `asset_custom_widget_samples` (25 hypertable candidates); `dropChunks` pre-filter for retention pruning. Boot-time detection caches hypertable status; subsequent `isHypertable()` checks return cached value without round-tripping.
