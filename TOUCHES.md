...
**Public API:** `buildAlertCharts(assetId, tokens, {now?, thresholds?, lossWindowMs?})`, `chartTokensIn(...templates)`, `substituteChartTokens(body, charts, {html})`, `attachmentsFor(charts, body)`, `lossBucketMs(windowMs)`, `CHART_TOKENS`, `CHART_WINDOW_MS`.
