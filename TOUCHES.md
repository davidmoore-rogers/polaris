...
- **Asset click-through is in-place and depends on assets.js being loaded.** Tapping an asset box (and the info rail's "Open asset details" button) calls `openViewModal` from `public/js/assets.js` — which is why `appmap.html` loads assets.js + its UI deps (table-sf / favorites / integrations), same stack as map.html. Dropping those script tags silently degrades the map to rail-only. Child process/service + external nodes are deliberately rail-only (the rail carries their per-node connection table).
