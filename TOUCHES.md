...
- The multi-process updater path auto-syncs shipped unit files into `/etc/systemd/system/` and runs `daemon-reload` before restarting (see `restartService()` in `src/services/updateService.ts`). When ADDING a new shipped unit file under `deploy/polaris-*.service` or `deploy/polaris.target`, also add its path to the sync `for f in ...` loop in `restartService()`, or the in-app updater won't pick it up on existing installs.
