...
**Resolving a tree to asset ids server-side:** `loadScopeAssetIds({condition})` (exported from [src/services/notificationEngine.ts](src/services/notificationEngine.ts)) for the narrow vocabulary — take the tree in a POST body and resolve there rather than having the client ship thousands of ids. The wide-vocabulary sibling is `contactService`'s own resolver.
