...
- **NotificationDelivery** — one outbound delivery per (notification, channel, concrete recipient). `channelId` → NotificationChannel (SetNull; null on a pending row = permanent fail) + `transport` (email/webhook/web_push/pushbullet) + `target` (recipient address/endpoint; "" for fixed-destination channels) + `meta` (web_push keys) + `status` (pending/sent/failed) + `attempts`. Destination secrets stay on the channel, read at send time. Expanded at fire time by `notificationRecipientService`; drained (≤3 retries) by `deliverNotifications`. Cascades with the Notification.
