...
- **AssetMonitorSample / AssetTelemetrySample / AssetInterfaceSample / AssetTemperatureSample / AssetStorageSample / AssetIpsecTunnelSample / AssetPerfSlaSample / AssetSdwanRuleSample** — eight append-only sample tables. Each has `*Hourly` and `*Daily` rollup companions populated by `runSampleRollup` (16 rollup tables total). The last two are the SD-WAN streams (FortiOS only, gated by `Integration.config.pullSdwan`): `AssetPerfSlaSample` = per (health-check, WAN-member) latency/jitter/packet-loss gauges + SLA thresholds; `AssetSdwanRuleSample` = per service-rule member-selection timeline (+ criteria / health-checks / destination / enabled config). Both ride the system-info cadence like IPsec.
