...
| Graph layout | Cytoscape.js for the Device Map topology modal; nodes positioned by a deterministic Dijkstra-weighted **column solver** (`computeTopologyColumns` in `topology-render.js`). Column **anchors** (FortiGate + switches + branch APs — mesh roots / bridged-switch parents) land on even columns by weighted depth; **leaves** (wired endpoints, wireless stations, and *terminal* managed APs) hang in the odd column right of their parent. Rows come in two passes: a **nearest-free-slot packing** pass over the anchors (first anchor child continues the parent's row so chains stay flat; every other anchor scans downward from just below its parent's row for the first lane where its whole footprint fits — its own cell, the spine chain that inherits its lane into later columns, and each chain member's planned AP block — so rows pack tight near the spine and spread only where subtrees congest, instead of every subtree pre-claiming a disjoint band), then a leaf pass that slides each parent's leaves as one **contiguous window** starting at the parent's row (one below it when its spine continues), blocks ordered by parent row — so a switch's APs line up directly beneath it, grouped together, never split apart or interleaved by another switch's APs. **Location codes** (`b:`/`f:`/`r:`/`jb:` parsed from device descriptions/notes by `src/utils/locationCodes.ts`) additionally cluster non-spine sibling anchors sharing a building (then floor) into adjacent row bands, and draw **grouping hulls** as synthetic bottom-layer Cytoscape nodes (nested rounded rectangles color-coded per level — building sky-blue solid > floor violet dashed > room green solid > jb amber dashed, fitted inside-out with a sibling-separation sweep so shapes never overlap; plain nodes, not compound parents, so `cy.png()` screenshots capture them); when any device carries `f:`, the modal offers per-(building, floor) **floor views** (Flat default) with dashed **portal** stubs where an edge crosses floors (tap → jump; `computeFloorViews` / `partitionElementsForFloor`). dagre + cytoscape-dagre (bundled under `public/js/vendor/`) remain the fallback when there's no FortiGate root |
