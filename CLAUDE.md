...
nginx terminates TLS for every install. `POLARIS_PROXY_CERT_PATH=/etc/polaris-nginx/cert.pem` + `POLARIS_PUBLIC_URL=https://polaris.example.com` are required in `/opt/polaris/.env` (boot fails fast otherwise). Polaris listens HTTP-only on `127.0.0.1:${PORT:-3000}` (web role); `TRUST_PROXY` defaults to `"1"` (first-hop trust) so `req.ip`/`req.secure` honor nginx's `X-Forwarded-*` headers, and Server Settings → Certificates renders an informational pane with the cert's fingerprint + SANs + expiry (no upload/generate/apply controls — operator manages the cert file directly). CA records (`category="ca"`) remain operator-editable because outbound TLS to integrations (LDAP, SMTP) still uses them.
