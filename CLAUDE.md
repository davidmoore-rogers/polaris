...
**FMG/FortiGate inline-merge fields not yet in projection** (potential 4f follow-up):
- `lastSeenSwitch`, `lastSeenAp` — only fortigate-endpoint observes these; current "stamp inline, never project" rule is correct but undocumented.
- The five FMG/FortiGate discovery pathways (DHCP-reservation / DHCP-lease / device-inventory / MAC-table / ARP) each have their own conservative merge logic in `syncDhcpSubnets` (e.g. ARP fills empty `ipAddress` only). Those rules don't go through projection — moving them in is a deeper refactor.
