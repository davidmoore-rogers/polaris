...
- `POST   /assets/bulk-monitor`                 *(assets admin)* — `{ ids, monitored, monitorType?, monitorCredentialId?, monitorIntervalSec?, probeTimeoutMs? }`. Applies one type+credential+timeout to every selected row, including integration-discovered firewalls and AD hosts. Sets `monitoredOperatorSet=true` on every touched row so subsequent discovery cycles don't re-flip `monitored`. Returns `{ updated, errors: [{id, error}] }`.
