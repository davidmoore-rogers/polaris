...
Assets converge via the multi-source model: every discovery pathway upserts `AssetSource` rows keyed on `(sourceKind, externalId)`, then `projectAssetFromSources()` derives the projected Asset row (hostname, serialNumber, manufacturer, model, os, osVersion, learnedLocation, ipAddress, lat/long, snmpLocation) by priority — `polaris-agent` (host-truth) > AD-FQDN > Intune > Entra > FortiGate sources. Inline writers still update `Asset.macAddress` / `assetType` / operator-owned fields.
