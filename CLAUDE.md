...
| Graph layout | Cytoscape.js for the Device Map topology modal; nodes positioned by a deterministic Dijkstra-weighted **column solver** (`computeTopologyColumns` in `topology-render.js`; rows assigned by a tidy-tree band pass over the predecessor tree — one device per row, first child continues the parent's row) — dagre + cytoscape-dagre (bundled under `public/js/vendor/`) remain the fallback when there's no FortiGate root |
