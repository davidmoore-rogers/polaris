...
│       ├── table-sf.js
│       ├── mobile/                  # Mobile SPA bundle (loaded by mobile.html). router.js (hash routing), auth.js (login + TOTP screens), map-tab.js (Device Map tab — lazy-loads Leaflet/markercluster, plots health-coded firewall pins, opens a bottom sheet on tap; backs both #map and #site/<id>), assets-tab.js (Assets tab — paged card feed with type filter chips, tap-through to asset detail), tabs.js (per-tab registry + Search renderer + shared snackbar helper), details.js (detail-route renderers — asset / subnet / block / site; site delegates to map-tab), app.js (orchestrator: bootstraps auth, mounts the tab shell, dispatches tab and detail routes; detail specs may carry a `parentTab` to keep the corresponding navbar item highlighted).
...
Asset
  ...
  monitorStatus   String?         -- "up" | "warning" | "pending" | "down" | "unknown" — see "Monitor Settings Hierarchy" / "Five-state monitor machine".
  ...
  consecutiveSuccesses Int        @default(0) -- Drives pending/warning -> up. Reset to 0 on any failure; failureThreshold doubles as the recovery threshold (same number of confirmations gates up <-> down both ways).
...
- **Tier-2 class overrides** — two surfaces share the same `MonitorClassOverride` table; either edits the same rows:
  - The integration's Monitoring tab → **Class Overrides** section (scoped to that integration; the asset-source picker is locked).
  - The Assets-page Monitoring Settings modal → **Class Overrides** section (cross-source view; asset-source picker offers every integration plus Manual).
...
The integration's Monitoring tab additionally carries a **Discovery Defaults** section on FMG/FortiGate types only, with three subtabs (FortiGates / FortiSwitches / FortiAPs). Each subtab holds discovery-time concerns: `addAsMonitored`, `snmpCredentialId`, `autoMonitorInterfaces`, plus the FortiGates-subtab-specific per-stream transport overrides. **Cadence inputs are no longer in these subtabs** — they live in the Cadence & Retention section above and apply uniformly across the integration's entire fleet.
...
- `POST   /server-settings/queue-mode`          — Body `{ mode: "cursor" | "pgboss" }`. Writes `Setting.monitor.queueMode` and returns the same shape as GET. Driven by the [Enable on next restart] button on the `pgboss_recommended` capacity reason. 409 if mode is "pgboss" but the package isn't installed in this build. Writes one `monitor.queue_mode.set` Event.
...
**Polling-method redesign (resolved per stream from a four-tier hierarchy).** There is no longer a per-asset `monitorType` column. Each of the four streams (response-time probe / telemetry / interfaces / LLDP) gets a polling method picked from a hierarchy: per-asset `*Polling` columns → `MonitorClassOverride.*Polling` → `Integration.config.monitorSettings.polling.*` (or the manual-tier `manualMonitorSettings.*Polling` Setting for orphan assets) → source default. Source defaults: FMG/FortiGate → `rest_api` on every stream; AD / Entra ID / Windows Server / Manual → `icmp` for response-time, null (= "not delivered") on the other three. The resolver enforces the compatibility matrix from `utils/pollingCompatibility.ts` (FortiManager / FortiGate accept `rest_api`, `snmp`, `ssh`, `icmp`; AD / Entra / Windows Server accept `icmp`, `winrm`, `ssh`; Manual accepts everything) — methods that don't apply to the asset's source kind are silently ignored at resolution time, and the route layer rejects them at write time with a clear 400.
...
- **Class override** — Assets page → Monitoring Settings modal → Class Overrides, OR the per-integration Monitoring tab's Class Overrides list. Methods re-render when the source picker changes; integration-scoped overrides are pinned to the integration's source kind.
