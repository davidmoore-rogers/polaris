...
**Ownership model for networks and reservations.** `subnets` and `reservations` carry an ownership dimension: `write` lets the caller edit only rows where `createdBy` matches their username; `fullwrite` bypasses the ownership filter. Built-in `user` and `assetsadmin` are seeded with `write`, `networkadmin` and `admin` with `fullwrite`. Route handlers branch on `req.permissionLevel` (set by `requireOwnership(functionKey)`). **Carve-out for reservations:** the ownership filter exists to stop write-level users from stepping on *each other's* manual reservations, so it does **not** apply to **discovery-created** reservations that no real user owns — `createdBy` null on the FMG/FortiGate sync path, or `"refresh"` from the subnet-refresh sweep. A write-level user may edit/release/retry those — this is what makes the IP-panel "Reserve" take-over of a DHCP lease work (the take-over flow releases the discovered lease before creating the manual reservation). `"system:*"` actors (e.g. `system:dns-resolved`) are **deliberately excluded** — dns_resolved rows stay view-only for non-admins, and their take-over goes through `createReservation` → `releaseDnsResolvedAt()` internally, not the gated DELETE. Encoded in `canMutateReservation()` in `src/api/routes/reservations.ts`, mirrored client-side in `public/js/ip-panel.js` + `public/js/mobile/reservations-tab.js`. (The `owner` column is unrelated display metadata — it carries the matched asset's `assignedTo`, not a Polaris user.)
