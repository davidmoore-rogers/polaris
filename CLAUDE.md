...
- **AssetMonitorSample / AssetTelemetrySample / AssetInterfaceSample / AssetTemperatureSample / AssetStorageSample / AssetIpsecTunnelSample / AssetPerfSlaSample / AssetSdwanRuleSample** — eight append-only sample tables. Each has `*Hourly` and `*Daily` rollup companions populated by `runSampleRollup` (16 rollup tables total). The last two are the SD-WAN streams (FortiOS only, gated by `Integration.config.pullSdwan`): `AssetPerfSlaSample` = per (health-check, WAN-member) latency/jitter/packet-loss gauges + SLA thresholds; `AssetSdwanRuleSample` = per service-rule member-selection timeline (+ criteria / health-checks / destination / enabled config). Both ride the system-info cadence like IPsec. **These sample/rollup tables (+ `AssetCustomWidgetSample`) carry `assetId` but NO foreign key to Asset** (cascade dropped in migration `20260615000000`): they're TimescaleDB hypertables, and a cascade DELETE matching a compressed chunk decompresses it into un-truncatable bloat (prod incident 2026-06-08). Deleting an Asset orphans its sample rows (queried only by assetId, never surfaced) to age out via `drop_chunks`. Never re-add the relation, and never row-DELETE/UPDATE sample rows that could sit in a compressed chunk — see [TOUCHES.md → tiered-sample-retention](TOUCHES.md).
