...
Phase 1 of the multi-phase plan to move TLS termination out of Node and into an external nginx reverse proxy. Activated by setting `POLARIS_PROXY_CERT_PATH=/etc/polaris-nginx/cert.pem` (+ `POLARIS_PUBLIC_URL=https://polaris.example.com`) in `/opt/polaris/.env`. When set: Polaris listens HTTP-only on `127.0.0.1:${PORT:-3000}` (web role), the Node HTTPS listener never binds, `TRUST_PROXY` auto-defaults to `"1"` (first-hop trust) so `req.ip`/`req.secure` honor nginx's `X-Forwarded-*` headers, and Server Settings → Certificates renders the in-app nginx GUI (cert rotation + 6 operator-settable directives) instead of the legacy upload/generate controls. CA records (`category="ca"`) remain operator-editable because outbound TLS to integrations (LDAP, SMTP) still uses them.

**In-app nginx GUI (proxy-mode-only).** Server Settings → Certificates exposes the six directives operators actually need to change day-to-day: HTTPS listen port, HTTP/3 toggle, TLS protocols, HSTS, Prometheus allow-list, plus cert+key rotation. `server_name` is derived from `POLARIS_PUBLIC_URL` and is not user-settable. The GUI persists settings to a `proxyConfig` Setting row and applies via a narrow privileged path: `polaris` user → `sudo /usr/local/sbin/polaris-nginx-apply <subcommand>` → wrapper holds `flock /run/polaris-nginx-apply.lock`, takes the staged file from `/run/polaris-nginx-stage/`, runs `nginx -t`, atomic-renames, reloads nginx. The wrapper is the entire privileged surface — sudoers doesn't restrict args, all validation lives in the script ([deploy/scripts/polaris-nginx-apply.sh](deploy/scripts/polaris-nginx-apply.sh)). Existing installs upgrade through the in-app update path: [src/services/updateService.ts](src/services/updateService.ts) cp-on-diff syncs the wrapper + `/etc/sudoers.d/polaris-nginx` + `/etc/tmpfiles.d/polaris-nginx.conf` + `usermod -aG nginx polaris` on every restart, so the helpers land without operator action. **Adopt-managed-mode gate**: `proxyConfig.managedMode` starts at `false` on every existing install; the GUI shows a yellow drift banner (with up to 5 specific drift markers from the regex parser in [src/services/nginxConfigParser.ts](src/services/nginxConfigParser.ts)) and disables the controls until the operator clicks "Adopt managed mode." This prevents silent clobbering of hand-edits beyond the 6 controls. In-app updates render `/etc/nginx/conf.d/polaris.conf` from the template only when `managedMode=true`; they also check sha256 against `lastAppliedHash` and refuse-to-clobber on drift.

**Cert rotation re-pin warning.** The rotate-cert flow in the GUI fires a libcrypto SPKI pair check, queries the count of currently-enrolled agents pinned to the current fingerprint (`count(*) FROM managed_agents WHERE install_status='active' AND server_cert_fingerprint=<current>`), and surfaces that count in a confirmation modal. The operator must explicitly confirm to proceed. **Phase 2 dual-pin / CA-pin is the actual fix**: until that ships, every cert rotation un-enrolls every active agent until reinstalled. The Polaris Agent's leaf-cert pinning works unchanged across rotation: `getServerCertFingerprint()` in [src/services/certInfo.ts](src/services/certInfo.ts) reads from `POLARIS_PROXY_CERT_PATH`, and the rotation flow explicitly calls `invalidateCache()` so the new fingerprint surfaces immediately.
