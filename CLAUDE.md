...
- `GET    /api-tokens`                          — List all tokens (`{ tokens[], knownScopes[] }`). Hashes are never returned.
- `POST   /api-tokens`                          — Create. Body: `{ name, scopes: string[], expiresAt? }`. Returns `{ token: ApiTokenSummary, rawToken: string }` — `rawToken` is shown once and never recoverable.
...
- Server Settings → **API Tokens** tab (admin-only, lazy-loaded) lists all bearer tokens with status badges (Active/Revoked/Expired), last-used IP, scopes, and Revoke/Delete actions. Create-token form has name, scope checkboxes (from `knownScopes`), and optional expiry. On creation a modal shows the raw token value with a Copy button — the only time it is visible.
