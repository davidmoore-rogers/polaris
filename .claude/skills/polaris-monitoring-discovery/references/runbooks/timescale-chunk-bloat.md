...
- `polaris-domain-model` → `samples-rollups.md`, the note on the sample tables ("These sample/rollup tables … carry `assetId` but NO foreign key to Asset … never row-DELETE/UPDATE sample rows that could sit in a compressed chunk").
- `polaris-business-rules` (rule 20c); `polaris-deploy` → `env-vars.md` (`TIMESCALE_COMPRESS_AFTER_DAYS`, `POLARIS_DB_DIRECT_URL`).
- `polaris-change-impact` → `cross-cutting/tiered-sample-retention.md` and `services/samples-timeseries.md` (`timescaleService.ts` entry).
