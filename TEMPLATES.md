...
- **Shared data fetch.** Built-in widgets that read `/dashboard/summary` accept it via the `summary` arg on `fetchData`; the NOC set shares `/dashboard/noc-summary` via `PolarisWidgets.getNocSummary(opts)` — 15s-memoized **per filter** (asset types + region scope), so widgets with the same filter share one fetch.
