...
**Key conventions:**
- **Column model (v2).** Layout = `{version:2, columns:[{id, width, widgets:[{id,type,height,config}]}]}`. Columns are CSS-grid items spanning their `width ∈ {3,4,6,12}` (12-col canvas; columns past 12 wrap); widgets stack vertically inside a column and carry `height ∈ {1,2}` set as a **fixed pixel height** on the article (280/576px) so chart/Leaflet bodies get a sized container. Array order IS the layout — there is no free-grid reflow (the old `reflow()`/`insertIndexFromCursor()`/grid-overlay/resize-grip are gone).
