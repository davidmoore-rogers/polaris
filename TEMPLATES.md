...
- **Frozen header on a list page** (canonical: Assets): add `.table-wrapper-sticky` to the `.table-wrapper` (turns it into the vertical scroll container + pins `thead th` with `position:sticky`; a box-shadow stands in for the th bottom border, which doesn't travel with sticky cells under `border-collapse`) and bound the wrapper to the viewport from the page's JS (`_sizeAssetsTableWrapper` in assets.js — `max-height = innerHeight − document-space top − reserve for the bottom pagination row`; re-run on window resize and after each render, since the empty top pagination row grows when controls first render). Scrolling then happens *inside* the wrapper, so the bulk bar + top pagination freeze in place for free. Filter popovers are `position:fixed`, so they're unaffected by the inner scroll.
