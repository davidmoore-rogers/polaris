...
**Key conventions:**
- **Column model (v2).** Layout = `{version:2, columns:[{id, width, widgets:[{id,type,height,config}]}]}`. Columns are CSS-grid items spanning their `width ∈ {3,4,6,12}` (12-col canvas; columns past 12 wrap); widgets stack vertically inside a column and carry `height ∈ {1,2,3}` set as a **fixed pixel height** on the article (280/576/872px) so chart/Leaflet bodies get a sized container. Array order IS the layout — there is no free-grid reflow (the old `reflow()`/`insertIndexFromCursor()`/grid-overlay/resize-grip are gone).
- **Explicit edit mode.** `state.editing` gates all chrome; "Customize Page" → `enterEditMode()`, "Done Editing" → `exitEditMode()` (forces the debounced save). Toggling re-renders the canvas (which also re-runs widget `onUnmount`/mount).
- **v1→v2 migration.** Legacy v1 layouts (`{widgets:[{col,row,...}]}`) are migrated on load by `migrateV1ToV2` (bucket by col-start, order by row); empty fallback on error. GET is server-unvalidated so stored v1 rows still load; the next save persists v2.
...
- **Shared data fetch.** Built-in widgets that read `/dashboard/summary` accept it via the `summary` arg on `fetchData`; the NOC set shares `/dashboard/noc-summary` via the 15s-memoized `PolarisWidgets.getNocSummary()`.
...
- **Shared row renderer for near-identical widgets.** The four top-N metric widgets (CPU / Memory / Slowest Response / Packet Loss) are structurally identical, so the row+bar render lives once in `public/js/widgets/_topnBar.js` (`window.PolarisTopN.renderRows/renderConfig`, reusing the `.util-bar-track`/`.util-bar-fill` bar from blockUtilization). Each widget becomes a ~30-line wrapper supplying unit / color thresholds / empty text. Underscore-prefixed filename = shared helper that registers no widget; include its `<script>` BEFORE its consumers.
