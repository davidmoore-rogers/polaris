...
- **Shared multi-widget fetch (NOC pattern).** When several widgets read the SAME endpoint on their own refresh timers, don't let each one fetch independently — add a memoized accessor on the registry (`PolarisWidgets.getNocSummary(opts, feeds)` in `public/js/widgets/index.js`: ~15s TTL + in-flight dedupe, keyed per (feeds, filter, limit)) and have every widget call it with only the feed(s) it renders. The backend endpoint (`/dashboard/noc-summary?feeds=`) computes just the requested subset via a feed registry in a dedicated service (`nocDashboardService.getNocSummaryPayload`), per-section permission-filtered like `/summary`, with every (feed, filter, cap) result served through a 10s `createTtlCache` (`src/utils/ttlCache.ts`) — the model for caching any read-mostly aggregate endpoint hit by many concurrent viewers.
