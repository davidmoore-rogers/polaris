...
- **Drag-to-reorder lives in the chooser, not the header.** Each chooser row carries a `.sf-col-grip` (`draggable="true"`, same idiom as the automations condition builder's `.aw-grip`, with `.sf-drop-before` / `.sf-drop-after` edge cues); dropping rewrites the order and a "Reset column order" button appears while it differs from the authored one. Reordering **physically moves** the `<th>`, its `<col>`, and every body row's cells — there is no CSS way to permute table columns — which is why:
...
- **Frozen header on a list page** (Assets / Events / IPAM Blocks + Networks, incl. the legacy blocks/subnets pages): add `.table-wrapper-sticky` to the `.table-wrapper` (turns it into the vertical scroll container + pins `thead th` with `position:sticky`; a box-shadow stands in for the th bottom border, which doesn't travel with sticky cells under `border-collapse`). The shared `sizeStickyTableWrappers()` in app.js bounds every such wrapper to the viewport (`max-height = innerHeight − document-space top − reserve for the bottom pagination row`); it's hooked into `renderPageControls` / `clearPageControls` (re-measuring after every list render, since the empty top pagination row grows when controls first render) and window resize — so for a page using `renderPageControls`, adding the class is the ONLY step. Pages with their own pagination renderer (Events) call it at the end of that renderer. Scrolling then happens *inside* the wrapper, so bulk bars + top pagination freeze in place for free. Filter popovers are `position:fixed`, so they're unaffected by the inner scroll. Not loaded on /dash (no app.js) — don't add the class to dash-rendered tables. **Slide-over variant:** inside a slide-over use `.table-wrapper-panel-sticky` instead (the asset-details Services tab is the reference) — same sticky-thead CSS, but the wrapper is height-bounded to the slide-over body's visible bottom by its own sizer (`_sizeAssetSvcTableWrapper` in assets.js, re-run on every tab activation + after each data load + window resize). Never put `.table-wrapper-sticky` in a slide-over: `sizeStickyTableWrappers()`'s triggers (list re-renders) fire while the panel tab is `display:none`, where rects are garbage, and would stamp bogus heights. **Modal variant:** inside a modal use `.table-wrapper-modal-sticky` (the Application Map → Discovery picker is the reference) — same sticky-thead CSS, but SELF-BOUNDING: the caller sets its own `max-height` (e.g. `46vh`) and there is deliberately no sizer, because a modal body already scrolls and its height isn't a function of the viewport top. Same reason as the slide-over: `.table-wrapper-sticky` would stamp a viewport-derived inline `max-height` over the modal's own bound on every resize / page-control render.
...
- **The auto-fill column is always explicitly pinned, never `width:auto`:** `pinAutoFillColumn` (scheduled after every `applyWidths`, re-run by a self-disconnecting `ResizeObserver` watching the table AND its parent — the parent because a container shrink doesn't change a fully-pinned table's own size) computes `container content width − Σ other visible columns' SPECIFIED widths` and writes it (only on change, so the observer doesn't re-fire). Arithmetic, not measure-based, for three reasons. (1) **Scroll-clamp ratchet:** transiently clearing the column's width to "measure the auto-fill result" shrinks the table for one layout pass; when the table overflows horizontally that clamps the wrapper's `scrollLeft` and yanks the view left on every drag frame. Never transiently un-size the column (`applyWidths` doesn't touch it). (2) **Colspan first `<tbody>` row** (interfaces table's section header): an auto-width column has no per-column width basis, so fixed layout strands the leftover as a trailing gap instead of growing the column. (3) **Distribution circularity:** when specified columns sum narrower than the table's `width:100%`, fixed layout distributes the leftover across *every* column, so rendered widths can't be trusted as inputs — summing them makes the computation self-fulfilling. The **crushed** case (other visible columns already sum past the container → leftover < 40px / `MIN_LAST_COL_W`) falls back to the column's saved/measured width so it stays visible and the wrapper's scrollbar reaches it — never a ~0px collapse whose header content paints into unreachable overflow.
- **Fixed utility columns** (`cb-col` checkbox / `fav-col` favorite-star) are non-resizable: they get no drag handle, are skipped by `lastVisibleResizableIdx` (never the auto-fill column), and are pinned to a fixed 20px (`FIXED_COL_W`) so they never get distributed leftover space under `table-layout:fixed`. They're also always `required` (non-hideable).
- **Static-width columns** (`data-col-no-resize="true"` on the `<th>`) are the tier between utility and resizable: no drag handle, skipped by `lastVisibleResizableIdx`, but they keep their own width rather than the 20px utility pin. That width comes from the **inline `style="width:…px"` on the `<th>`, which is the single source of truth** — always declare one, and never rely on a measured render: `thead th` sets `overflow-wrap: anywhere`, so a header's min-content is one character, and an over-committed table's auto-layout pass can crush a declared 120px column to ~40px. With no handle to drag, such a measurement used to lock in permanently (and persist via `getPrefs`), so `setupColumnLayout` now pins these columns from the declared width up front and `setPrefs` ignores saved widths for them exactly as it does for cb/fav. To resize one, edit the inline width in the HTML. Use it for columns whose content is a fixed-size token — enum badges, MACs, timestamps (Assets Type / State / Status / Monitored Via / MAC Address; Events Timestamp / Level; IPAM CIDR / Version / Networks / Created / VLAN / Status / Reservations; Automations Severity / Enabled).
...
- **Header pill + severity-tier CSV export.** `PolarisWidgets.setHeaderCount(el, count)` stamps the red overall-total pill on the widget header; `PolarisWidgets.setHeaderExport(el, {columns, rows, filename?, severityOf?})` stamps a ⌄-menu ⤓ export button whose tiers (All rows / Critical only / Serious and up / Warning and up, live counts) filter the widget's FULL pre-clip listing on each row's active-alert severity (`ALERT_SEVERITY_RANK`, the client mirror of nocDashboardService's ladder — event levels fold in at error=critical / info=informational) and download a CSV with an auto-appended Severity column (filename defaults from the widget `data-type`; local CSV fallback on the Dash wallboard, which lacks app.js `downloadCsv`). Both are re-call-every-render-tick helpers that no-op outside a dashboard shell (library previews) and remove themselves at zero rows/count. Export is wired on Active Alerts, Down Nodes, Down Interfaces, Storage Forecast, and all six top-N widgets (once, inside `_topnBar.renderRows`) — a new severity-bearing listing widget should call it with its own column spec.
...
- **Discovery scheduler.** `discoveryScheduler.ts` invokes the same per-type dispatch as the manual discover route. Add the new type next to the others.
...
- `_polarisSourceLabel(source, opts)` — the per-source-kind name table: FortiGate Direct / FortiManager Proxy / Active Directory / Entra ID / Windows Server / Manual.
