...
- Drag-to-resize is **independent**: dragging a handle changes only that column's width (40px floor); the columns to its right shift over. The **rightmost visible, resizable column is left auto-width** (`lastVisibleResizableIdx`) so it absorbs the slack and its right edge stays pinned to the table border — its own handle is hidden/inert, so the operator can't drag the table's right edge off the border. When the fixed columns grow wider than the container the table overflows past `width:100%` and the wrapper's `overflow-x:auto` shows a horizontal scrollbar. `applyWidths` recomputes the auto-fill column on every resize and whenever the chooser hides/shows a column.
