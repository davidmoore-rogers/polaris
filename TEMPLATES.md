...
- **Colspan-first-row caveat:** `table-layout:fixed` sizes an auto-width column from its cell in the first `<tbody>` row. When that row is a `colspan` cell (e.g. the interfaces table's `sectionRow("Interfaces", n)` header), the auto-fill column has no per-column width basis and fixed layout strands the leftover as a trailing gap to the right of the last column. `pinAutoFillColumn` (scheduled after `applyWidths`, also re-run by a self-disconnecting `ResizeObserver` on reveal/resize) detects that gap and pins the column to an explicit computed width (`tableWidth − Σ other visible cols`). It's a no-op when auto-fill already reaches the border *with a usable width*, so non-colspan tables stay on responsive `width:auto`. The same function also rescues the **crushed auto-fill column**: when the other visible columns' widths sum past the container, fixed layout leaves no leftover and the auto column collapses to ~0px — its right edge still meets the table border (so edge-distance alone reads as success), but the column is invisible and its header content paints past the table border into overflow the wrapper's scrollbar can't reach. `pinAutoFillColumn` detects width < 40px (`MIN_LAST_COL_W`) and pins the column's saved/measured width so the table grows and the horizontal scrollbar covers it.
