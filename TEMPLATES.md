...
**Canonical implementation:** the Automations 6-step builder — `openAutomationWizard(existing)` in `public/js/automations-wizard.js` (page module: `public/js/automations.js`). Second instance: the Application Map map-rule wizard — `openAppMapRuleWizard(existing)` in `public/js/appmap-rules-wizard.js` (list module: `public/js/appmap-discovery.js`), a smaller 4-step copy of the same idiom; read it when the automations one is more machinery than you need. **Nesting caveat it demonstrates:** `openModal` reuses ONE overlay, so a wizard opened from a list modal REPLACES it — the map-rule wizard's Cancel and post-save paths reopen the list instead of calling `closeModal()`. Shared CSS: `.stepper` / `.stepper-step{.active,.done,.clickable}` / `.stepper-num` / `.stepper-line` / `.step-panel{.visible}` + the sticky `.modal-body > .stepper` rule in `public/css/styles.css` (promoted from setup.html's inline copy — the pre-auth setup wizard keeps its own, deliberately self-contained). Companions: `.aw-sentence` (live plain-English summary callout) and `.review-grid` (final summary card).
