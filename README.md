...
**PostgreSQL tuning for large deployments**: open Server Settings → Maintenance → **Capacity Advisor** after the host has been monitoring for ~15 minutes. It computes recommended values for `shared_buffers` / `work_mem` / `effective_cache_size` / `random_page_cost` / `track_io_timing` / `max_connections` based on observed workload, host RAM, and connection peak, then surfaces them alongside Polaris's own pool and worker tunables. For headless installs, see `docs/INSTALL.md` → "Capacity tuning — use the Capacity Advisor."
