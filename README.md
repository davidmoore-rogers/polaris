...
- **Response-time probe** (default 60 s) — FortiOS REST, SNMP `sysUpTime`, WinRM SOAP, SSH connect+auth, an **HTTP check** (one GET against a path you choose, judged on the status code and optionally on a string the response body must contain), ICMP, or **Polaris Agent**. Records round-trip time on success and `null` on failure ("packet loss"). A missed probe puts the asset in `warning` and switches it to a fast **confirmation re-probe** (default 10 s, per-class/integration setting) so Down is confirmed in seconds rather than in multiples of the poll interval; the same applies to confirming a recovery. Down/up transitions emit audit events and drive the sidebar status pill.
